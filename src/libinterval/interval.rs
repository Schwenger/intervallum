    ...
    Bound: Width,
{
    fn new(lb: Bound, ub: Bound) -> Interval<Bound> {
        debug_assert!(lb >= <Bound as Width>::min_value(), "Lower bound exceeds the minimum value of a bound.");
        debug_assert!(ub <= <Bound as Width>::max_value(), "Upper bound exceeds the maximum value of a bound.");
        Interval { lb, ub }
    }
}
...
where
    ...
    Item: Ord,
{
    debug_assert!(iter.size_hint().0 > 2, "`min_max` expects an iterator (`iter`) yielding at least two elements.");
    let (mut min, mut max) = {
...
mod tests {
    ...
    #[test]
    fn is_subset_test() {
        let cases = vec![(zero, zero, true), (i1_2, i1_2, true), (empty, empty, true), (invalid, invalid, true)];
    ...
    fn is_proper_subset_test() {
        let cases = vec![(zero, zero, false), (i1_2, i1_2, false), (empty, empty, false), (invalid, invalid, false)];
    ...
    fn intersection_test() {
        let cases = vec![(zero, zero, zero), (i1_2, i1_2, i1_2), (empty, empty, empty), (invalid, invalid, invalid)];
    ...
    #[test]
    fn is_disjoint_test() {
        let cases = vec![(zero, zero, false), (i1_2, i1_2, false), (empty, empty, true), (invalid, invalid, true)];
    ...
    fn is_disjoint_option_test() {
        let mut cases: Vec<(u32, Interval<i32>, Optional<i32>, bool)> =
            is_disjoint_cases().into_iter().map(|(id, a, b, e)| (id, a, Optional::singleton(b), e)).collect();
        cases.extend(vec![
    ...
    fn test_lattice() {
        ...
        let tester = LatticeTester::new(
            ...
            /* a |= b*/
            vec![True, True, False, True, Unknown, Unknown, True, Unknown, Unknown],

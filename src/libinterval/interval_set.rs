    ...
    Bound: Width + Num,
{
    ...
    fn from_interval(i: Interval<Bound>) -> IntervalSet<Bound> {
        ...
        IntervalSet { intervals: vec![i], size }
    }
    ...
    fn join_or_push(&mut self, x: Interval<Bound>) {
        ...
        "This operation is only for pushing interval to the back of the array, possibly overlapping with the last element.");
            ...
            let joint = if joinable(self.back(), &x) { self.pop().unwrap().hull(&x) } else { x };
            self.push(joint);
        }
    }

    fn find_interval_between(&self, value: &Bound, mut left: usize, mut right: usize) -> (usize, usize) {
    ...
    {
        IntervalSet { intervals: self.intervals.iter().map(f).collect(), size: self.size.clone() }
    }
    ...
    where
        F: Fn(&Interval<Bound>) -> Interval<Bound>,
    {
        self.intervals.iter().fold(IntervalSet::empty(), |mut r, i| {
            r.push(f(i));
            r
        })
    }
...
impl<Bound: Width + Num> Empty for IntervalSet<Bound> {
    fn empty() -> IntervalSet<Bound> {
        IntervalSet { intervals: vec![], size: <<Bound as Width>::Output>::zero() }
    }
}
    ...
    fn complement(&self) -> IntervalSet<Bound> {
        ...
        if self.is_empty() {
            ...
            for i in 1..self.intervals.len() {
                ...
                res.push(Interval::new(previous.upper() + one.clone(), current.lower() - one.clone()));
            }
...
mod tests {
    ...
    fn test_inside_outside(is: IntervalSet<i32>, inside: Vec<i32>, outside: Vec<i32>) {
        for i in &inside {
            assert!(is.contains(i), format!("{} is not contained inside {}, but it should.", i, is));
        }
        for i in &outside {
            assert!(!is.contains(i), format!("{} is contained inside {}, but it should not.", i, is));
        }
    }
    ...
    fn test_binary_op_sym<F>(test_id: String, a: Vec<(i32, i32)>, b: Vec<(i32, i32)>, op: F, expected: Vec<(i32, i32)>)
    where
    ...
    fn test_binary_op<F>(test_id: String, a: Vec<(i32, i32)>, b: Vec<(i32, i32)>, op: F, expected: Vec<(i32, i32)>)
    where
        F: Fn(&IntervalSet<i32>, &IntervalSet<i32>) -> IntervalSet<i32>,
    {
    ...
    fn test_binary_value_op<F>(test_id: String, a: Vec<(i32, i32)>, b: i32, op: F, expected: Vec<(i32, i32)>)
    where
    ...
    fn test_binary_bool_op_sym<F>(test_id: String, a: Vec<(i32, i32)>, b: Vec<(i32, i32)>, op: F, expected: bool)
    where
    ...
    fn test_binary_bool_op<F>(test_id: String, a: Vec<(i32, i32)>, b: Vec<(i32, i32)>, op: F, expected: bool)
    where
        F: Fn(&IntervalSet<i32>, &IntervalSet<i32>) -> bool,
    {
        println!("Info: {}.", test_id);
    ...
    fn test_binary_value_bool_op<V, F>(test_id: String, a: Vec<(i32, i32)>, b: V, op: F, expected: bool)
    where
    ...
    fn test_overlap_and_is_disjoint() {
        ...
        for (id, a, b, expected) in sym_cases {
            ...
            test_binary_bool_op_sym(format!("test #{} of is_disjoint", id), a, b, |x, y| x.is_disjoint(y), !expected);
        }
    }
    ...
    fn test_overlap_bound() {
        ...
        for (id, a, b, expected) in cases {
            ...
            test_binary_value_bool_op(format!("test #{} of overlap_bound", id), a, b, |x, y| y.overlap(x), expected);
        }
    }

    #[test]
    fn test_overlap_option() {
        let mut cases: Vec<(u32, Vec<(i32, i32)>, Optional<i32>, bool)> =
            overlap_cases().into_iter().map(|(id, a, b, e)| (id, a, Optional::singleton(b), e)).collect();
        cases.extend(
            vec![
    ...
    fn test_arithmetics() {
        ...
        for (id, a, b, e_add, e_sub, e_sub_sym, e_mul) in cases {
            test_binary_op_sym(format!("test #{} of `a+b`", id), a.clone(), b.clone(), |x, y| x + y, e_add);
            test_binary_op(format!("test #{} of `a-b`", id), a.clone(), b.clone(), |x, y| x - y, e_sub);
            test_binary_op(format!("test #{} of `b-a`", id), b.clone(), a.clone(), |x, y| x - y, e_sub_sym);
            test_binary_op_sym(format!("test #{} of `a*b`", id), a.clone(), b.clone(), |x, y| x * y, e_mul);
        }
    }
    ...
    fn test_arithmetics_bound() {
        ...
        for (id, a, b, e_add, e_sub, e_mul) in cases {
            test_binary_value_op(format!("test #{} of `a+b`", id), a.clone(), b.clone(), |x, y| x + y, e_add);
            test_binary_value_op(format!("test #{} of `a-b`", id), a.clone(), b.clone(), |x, y| x - y, e_sub);
            test_binary_value_op(format!("test #{} of `a*b`", id), a.clone(), b.clone(), |x, y| x * y, e_mul);
        }
    }
    ...
    fn test_lattice() {
        ...
        let tester = LatticeTester::new(
            0,
            /* data_a */
            vec![empty.clone(), empty.clone(), whole.clone(), a.clone(), b.clone(), c.clone()],
            /* data_b */
            vec![whole.clone(), a.clone(), a.clone(), b.clone(), c.clone(), d.clone()],
            /* a |= b*/ vec![True, True, False, Unknown, False, Unknown],
            /* a |_| b */
            vec![empty.clone(), empty.clone(), a.clone(), e.clone(), c.clone(), f.clone()],
            /* a |-| b */
            vec![whole.clone(), a.clone(), whole.clone(), g.clone(), b.clone(), h.clone()],
        );
        tester.test_all();
    }
